...
export default class Cam extends Component {
  ...
  componentDidMount() {
    ...
    getData = async () => {
      ...
      try {
        if (this.state.unit == 'second(s)') {
          this.setState({intervalInMiliseconds: this.state.interval * 1000})}
        if (this.state.unit == 'minute(s)') {
          this.setState({intervalInMiliseconds: this.state.interval * 1000 * 60})}    
